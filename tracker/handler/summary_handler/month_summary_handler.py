...
class MonthSummaryHandler(SummaryHandlerInterface):
    def __init__(self):
        super(MonthSummaryHandler, self).__init__()
        self.__current_month_number = datetime.datetime.now().month

    def summary(self) -> pandas.DataFrame:
        self._data = self._entry_handler.get_data()

        self._data = self.add_column_for_month_number()
        self._data = self.drop_rows_with_entries_not_created_this_month()
    ...
    def add_column_for_month_number(self) -> pandas.DataFrame:
        data = self._data
        data["month_number"] = self._data["start_date"].map(self.__get_month_number)

        return data

    @staticmethod
    def __get_month_number(date: str) -> int:
        date = datetime.datetime.strptime(date, "%b, %d %Y")
        return date.month

    def drop_rows_with_entries_not_created_this_month(self) -> pandas.DataFrame:
        entries_to_drop = self._data.loc[self._data['month_number'] != self.__current_month_number]
